...
Foi aí que rolou um trabalho de detetive. Os engenheiros começaram a colocar etiquetas (_tags_) em cada operação SQL no código para descobrir quem estava "monopolizando" o banco. O resultado? O problema nem eram as reservas! Partes antigas do fluxo de checkout estavam segurando conexões por tempo demais, igualzinho aquele vizinho sem noção que ocupa a vaga da garagem e some. As reservas foram apenas a "gota d'água" em um pool de conexões que já estava no limite.
